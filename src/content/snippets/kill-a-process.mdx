...
<Callout type="info" title="Before you kill it, know why it hung">

The hung-process guide is the command set for the process `pkill` will not stop: the D state that ignores every signal, `/proc/PID/wchan`, and what to capture before `kill -9` destroys the evidence. Full guide: [Diagnosing a Hung Process](/guides/diagnose-a-hung-process).
...
`pkill` ends a process. It does not tell you why the process needed ending. When a job has to be killed on a schedule, the fix is upstream: a lock so runs cannot overlap, a `timeout` so a hung run reaps itself, and an ERR trap that reports the failure instead of leaving you to find it. [The Production Bash Toolkit](/starter-kit) composes those three into a single wrapper.
