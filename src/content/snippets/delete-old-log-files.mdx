...
<Callout type="info" title="This is one of the 25">

Log cleanup is the second script in the sysadmin set, right after the disk-space check that tells you it is needed. The guide puts the two on one cron schedule and adds the retention, backup and SSL checks that share the same failure: nothing pruned, nothing noticed. Full guide: [25 Bash Scripts Every Linux Sysadmin Needs](/guides/bash-scripts-every-sysadmin-needs).
...
If the directory holds dated folders rather than loose files, and you want to keep the newest N no matter how old they are, use [Log Retention Cleanup](/snippets/log-retention-cleanup) instead: it dry-runs by default and refuses a bad `--keep` or `--days` value before it deletes anything.
