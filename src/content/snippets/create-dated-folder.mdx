...
<Callout type="info" title="The naming convention every backup script uses">

Every backup script in the sysadmin guide writes into a dated folder like this one, because a date in the path is what lets a retention script decide what to delete. Full guide: [25 Bash Scripts Every Linux Sysadmin Needs](/guides/bash-scripts-every-sysadmin-needs).
...
Dated folders solve naming. They do not solve the run that dies halfway and leaves a half-written directory that looks complete to the next script down the chain. A `trap` that cleans up on every exit path, plus writing to a temp name and moving atomically once the work is done, are part of the operational layer in [The Production Bash Toolkit](/starter-kit).
