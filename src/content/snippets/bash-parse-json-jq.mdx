...
<Callout type="info" title="jq is step two of the three-step pattern">

The APIs guide is the full fetch → parse → alert script: curl that fails when the API fails, jq with `-e` so a missing field is an error, and a Slack alert when either breaks. Full guide: [Shell Scripts That Talk to APIs](/guides/shell-scripts-that-talk-to-apis).
