...
<Callout type="info" title="grep is step two of four">

The text-processing guide puts grep between `find` (scope the files first) and `sed`/`awk` (transform or summarize what grep found), with the incident pipeline that ranks errors in a log by count. Full guide: [Bash Text Processing: find, grep, sed, and awk](/guides/bash-text-processing).
