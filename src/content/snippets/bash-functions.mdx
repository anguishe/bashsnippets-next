...
<Callout type="info" title="local x=$(cmd) is the trap this guide explains">

The safe-template guide covers why `local x=$(false)` never triggers `set -e`, the two-line form that fixes it, and the ERR trap that reports the function and line that failed. Full guide: [The Safe Bash Script Template](/guides/safe-bash-script-template).
