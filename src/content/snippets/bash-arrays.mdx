...
<Callout type="info" title="Arrays and strict mode">

The safe-template guide covers the `nounset` rule that made `"${arr[@]}"` on an empty array a folklore error before bash 4.4, and the header every script iterating a list should start with. Full guide: [The Safe Bash Script Template](/guides/safe-bash-script-template).
