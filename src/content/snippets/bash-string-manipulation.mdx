...
<Callout type="info" title="When the string is a whole file">

Parameter expansion handles one value. The text-processing guide is the same job at file scale: sed for replace, awk for fields, and the anchoring rules that stop a bulk edit from rewriting substrings you never looked at. Full guide: [Bash Text Processing: find, grep, sed, and awk](/guides/bash-text-processing).
