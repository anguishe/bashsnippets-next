...
<Callout type="info" title="This script handles one of the three kinds of “down”">

`systemctl is-active` tells you whether the unit is running. It does not tell you whether the service is *answering*, whether someone stopped it on purpose, or why `systemctl start` is refusing with "start request repeated too quickly". The full guide covers all three, the exit code 4 that makes a watchdog restart a unit that does not exist, and a watchdog that alerts once per outage: [Auto-Restart a Stopped Service on Linux](/guides/auto-restart-linux-service).
...
The `>> /var/log/watchdog-cron.log 2>&1` redirect captures all output so you have a log of every check, every outage, and every recovery. A service watchdog is one of the core jobs in the [bash scripts every sysadmin needs](/guides/bash-scripts-every-sysadmin-needs) guide.
...
For long-running apps you control, a unit drop-in can set `Restart=on-failure` (and related options) so systemd restarts the process without cron. Cron still helps when you want mail, extra logging, or cross-checks across several units. For a cron watchdog that also probes a hung-but-running service, bounds start-limit resets and alerts once per outage instead of once a minute, see [Service Watchdog](/snippets/service-watchdog).
...
A watchdog that restarts a service is one crash-loop away from hiding the real problem. Bounding the restart count, logging every intervention with a timestamp, and escalating when the count climbs is the difference between a watchdog and a cover-up. [The Production Bash Toolkit](/starter-kit) implements that logic once, as a shared library, rather than per service.
