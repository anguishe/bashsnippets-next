...
<Callout type="info" title="The report tells you the box is busy — not why">

When the load is high and nothing obvious is running, the hung-process guide is the next command set: STAT letters, `/proc/PID/wchan`, and what to capture before you kill anything. Full guide: [Diagnosing a Hung Process](/guides/diagnose-a-hung-process).
...
If the question is only "which address is this box using", [Find Your IP Address on Linux](/snippets/find-ip-address-linux) covers local, public, gateway, DNS and MAC on their own.
