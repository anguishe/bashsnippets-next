...
<Callout type="info" title="Part of the disk-recovery set">

The sysadmin guide orders the disk work: find what is big, find what is duplicated, find what is old — and only then delete, with a dry run in front of every `rm`. Full guide: [25 Bash Scripts Every Linux Sysadmin Needs](/guides/bash-scripts-every-sysadmin-needs).
...
```bash
find "$TARGET" -maxdepth 3 -type f -print0 \
  | xargs -0 -r md5sum 2>/dev/null \
  | sort \
  | awk 'seen[$1]++'
```
