...
<Callout type="info" title="When the check itself needs to alert">

The APIs guide shows how to send this result somewhere — a Slack webhook with a jq-built payload — and how to make the sending step fail loudly instead of exiting 0 on a 500. Full guide: [Shell Scripts That Talk to APIs](/guides/shell-scripts-that-talk-to-apis).
...
An expiry check is only as good as the plumbing around it: the lock that stops overlapping runs, the log that proves last night's check happened, the alert that still reaches you when the mail host is the thing that died. That plumbing is what [The Production Bash Toolkit](/starter-kit) standardises — one shared library and one script template, instead of re-deriving it per monitor.
