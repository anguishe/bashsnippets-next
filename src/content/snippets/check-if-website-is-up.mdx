...
<Callout type="info" title="Same curl, hardened">

The APIs guide turns this one-liner into the production pattern: `--fail` so curl exits non-zero on an HTTP error, `--max-time`, a retry, and an alert to Slack instead of an echo nobody reads. Full guide: [Shell Scripts That Talk to APIs](/guides/shell-scripts-that-talk-to-apis).
...
| URL | `https://example.com/api/health` | You can point it at a specific endpoint, not just the homepage |
...
An uptime check that silently stops running is worse than no check at all, because you trust it. Making the monitor prove it ran — a lock, a bounded runtime, a timestamped log, and an alert that escalates when the check itself goes quiet — is the operational layer collected in [The Production Bash Toolkit](/starter-kit).
