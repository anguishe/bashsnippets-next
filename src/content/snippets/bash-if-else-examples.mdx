...
<Callout type="info" title="Every test here assumes strict mode">

The safe-template guide explains the `set -euo pipefail` header these examples rely on, and the ERR trap that names the failing line when a test you did not expect to fail does. Full guide: [The Safe Bash Script Template](/guides/safe-bash-script-template).
