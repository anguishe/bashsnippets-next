...
<Callout type="info" title="Permissions are one of four security checks">

The sysadmin guide runs this alongside the SSH hardening, failed-login and open-port checks, on a schedule, with the output mailed to you instead of printed to a terminal nobody is watching. Full guide: [25 Bash Scripts Every Linux Sysadmin Needs](/guides/bash-scripts-every-sysadmin-needs).
