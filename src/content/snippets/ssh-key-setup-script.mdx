...
<Callout type="info" title="Keys are step one of SSH hardening">

The sysadmin guide follows this with the sshd_config changes that actually stop password brute force — the part `ssh-copy-id` exiting 0 does not do for you. Full guide: [25 Bash Scripts Every Linux Sysadmin Needs](/guides/bash-scripts-every-sysadmin-needs).
...
Once key login works, [Run Commands on a Remote Server over SSH](/snippets/ssh-run-remote-commands) covers the next step: one command or a heredoc block on one host or twenty, with per-host exit codes.
