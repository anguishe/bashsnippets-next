...
<Callout type="info" title="High load is often a service that is up but stuck">

The service-watchdog guide covers the case this monitor exposes: a unit `systemctl` calls active that has stopped answering, and a watchdog that probes it instead of trusting the status. Full guide: [Auto-Restart a Stopped Service on Linux](/guides/auto-restart-linux-service).
...
Writing to a log file is pointless if nobody checks it. Combine this with the [Send Email Alerts from Bash](/snippets/bash-send-email-alert) snippet to get notified when thresholds trip. Or pipe cron output to your inbox. When sustained resource pressure is causing service crashes, the [restart service watchdog](/snippets/restart-service-if-stopped) handles automatic recovery between monitoring cycles. A service the kernel kills under memory pressure exits with code 137 — 128 plus signal 9 ([SIGKILL](https://man7.org/linux/man-pages/man7/signal.7.html)), the OOM killer's signature — so a 137 in your logs points to RAM exhaustion, not a code bug.
...
Sampling CPU and RAM takes six lines. Making the sampler survive a year of unattended cron takes considerably more — no overlapping runs, no silent death on a hung `free`, a log with timestamps you can correlate against the incident, and an alert that fires once rather than every minute. [The Production Bash Toolkit](/starter-kit) is that scaffolding, as a library and a script template.
