...
<Callout type="info" title="Find every listener, not one port">

The open-ports guide is the full audit — `ss -tlnp` without root, docker-proxy listeners that hide the real process, and a script that diffs today’s listeners against yesterday’s and alerts on new ones. Full guide: [List Open Ports on Linux](/guides/open-ports-linux).
...
The same `lsof -ti :PORT | xargs kill` command works on macOS. The [`ss`](https://man7.org/linux/man-pages/man8/ss.8.html) command is Linux-only (part of iproute2), but macOS ships with `lsof` pre-installed. Use `sudo` if the process runs as root or another user.
