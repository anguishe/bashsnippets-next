...
One more guard that costs nothing: run the finished script through ShellCheck before it goes into the crontab. The two findings it raises most on cron jobs are [SC2086](/shellcheck/sc2086) — an unquoted `$LOG_FILE` or `$LOCK` that splits on a space and writes somewhere else — and [SC2046](/shellcheck/sc2046), the unquoted `$(date ...)` inside a filename. Both are silent at 3am and loud in the decoder.
...
Every guard in this guide — `flock`, `timeout`, retry with backoff, `trap` cleanup, and logging that survives the run — is implemented once as a shared `bashlib.sh` in [The Production Bash Toolkit](/starter-kit), alongside a `template.sh` that starts each new job with all five already wired and a 52-page field guide covering the failure modes this page does not.
