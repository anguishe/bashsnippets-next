...
`set -u` turns an unset variable into an immediate error. The trouble is that "unset" is environment-dependent: a variable that is always present on your laptop and in production because it lives in a profile can be missing in a container, a CI runner, or a cron environment — three places nobody tests interactively. Why an unexported variable never reaches a child process, and why cron starts with almost none of yours, is in [Bash Environment Variables](/snippets/bash-environment-variables).
...
ShellCheck catches the static half of this before anything runs: [SC2154](/shellcheck/sc2154) when a lowercase variable is referenced but never assigned (a typo in the name), and [SC2034](/shellcheck/sc2034) when one is assigned and never read — the other half of the same typo. Neither fires on UPPERCASE names, which ShellCheck assumes are environment variables, so `set -u` stays the runtime guard for those. And the `rm -rf "${DIR:?}/"` form that `nounset` pushes you toward is the whole reason [SC2115](/shellcheck/sc2115) exists: an empty `DIR` there is not an error, it is `rm -rf /`.
