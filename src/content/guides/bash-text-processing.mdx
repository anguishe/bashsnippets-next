...
Three ShellCheck rules live in that pipeline. `xargs grep` breaks on a filename with a space — `find -print0 | xargs -0` is the fix — and the same word-splitting is what [SC2046](/shellcheck/sc2046) flags when you wrap `find` in `$( )` instead of piping it. The awk program is in single quotes on purpose: `$1` there is awk's field, not a shell variable, which is the one case [SC2016](/shellcheck/sc2016) knows to leave alone. And if you ever grep for a literal `*` and get nothing back, that is [SC2063](/shellcheck/sc2063): grep reads a regex, not a glob.
