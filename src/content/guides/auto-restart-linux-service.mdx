...
If after all that you still want a cron-driven watchdog — for a service without a sane `Restart=`, for the hung case on a daemon with no `WatchdogSec` support, or on a box where you do not control the unit files — this is the version that handles the three states, the exit-code-4 trap, the start-limit trap, a maintenance window, overlapping runs, and alert spam. It is the script behind the [Restart a Service If It Stopped](/snippets/restart-service-if-stopped) snippet, grown up, and it has its own page: [Service Watchdog](/snippets/service-watchdog).
