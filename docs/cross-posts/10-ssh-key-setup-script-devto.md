---
title: "One ssh Flag Tells You Whether a Server Still Accepts Passwords"
published: true
description: "ssh-copy-id exits 0 and the passwordless login works, and neither tells you whether password auth is still open. The PreferredAuthentications=none probe that does, why SSH hides key refusals from the client, and the key setup with the one test that doesn't lie."
