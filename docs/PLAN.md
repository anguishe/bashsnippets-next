...
Last updated: **2026-09-20**. Owner: Travis. Maintained by Claude Code.
...
| 3.C | Cross-posting (10 drafts + 3 new + 2 kit) | ✅ **2026-09-21: all 14 on dev.to + Medium + CoderLegion** (#01 live on all 3; 02–14 scheduled Tue+Thu 9/22 → 11/3, 08:00 Central, same minute on each platform) | `docs/cross-posts/queue/README.md` (order of record) + `POST-QUEUE-2026-09.md` (recipes) |
...
| 3.F | 2026-09-16 audit fixes: license wording, 30 functions, test claims, top buy button (`?wanted=true`), starter page upsell, 7 orphans linked, dead anchor | ✅ 2026-09-16, commits `869e096` + next, deployed, IndexNow 200 (79). Crawl: 0 broken links / fragments / orphans | `AUDIT-2026-09-16.md` |
| 3.G | Distribution capacity | ✅ **2026-09-20 — the whole queue is scheduled, not just drafted.** Posts 02–14 publish themselves at 08:00 America/Chicago on the queue dates (ids `4700404`–`4700422`). dev.to's API *can* schedule despite the docs: `PUT /api/articles/{id}` with `published: true` + a future `published_at` → SCHEDULED. Verified by anonymous fetch (404 on all 13) and by the public feed still showing #01 as newest. `scripts/devto-drafts.mjs` created them, `scripts/devto-schedule.mjs` dated them; both idempotent. **2026-09-21: Medium 02–14 + CoderLegion 02–14 also scheduled for the same minute. 39 posts, zero hand steps** | `MANUAL-ACTIONS-2026-09-20.txt` |
...
- **2026-09-21 (night) — old Medium canonicals fixed + C4/C5 guide.** 21 older published Medium stories had no canonical (Medium pointed at itself) or an old `.html` one; each now has `rel=canonical` to its bashsnippets.xyz page, mapped by exact title from the dev.to API. Two by the article's own primary link: For-Loop-Space → `/snippets/bash-read-file-line-by-line`, grep -rn → `/snippets/search-files-for-text-grep`. All 23 verified live. **C4 corrected:** Gumroad Discover requires $100 of genuine account sales + a ~3-week risk review + ≥1 sale of the product (Gumroad help center), so it cannot list the toolkit before 11/04. C4 is prep only (category/tags/payouts). **C5 odds lowered:** alebcay/awesome-shell's last merge was 2025-08-28. Click-by-click for both: `docs/GUMROAD-DISCOVER-AND-AWESOME-SHELL-GUIDE.txt`. PR 2's insert point corrected to after `critic.sh`.
- **2026-09-21 — distribution finished: Medium + CoderLegion scheduled alongside dev.to.** Claude drove the anguisheh1 Chrome. CoderLegion has a native *Schedule Post Time* (the 9/20 notes planned weekly hand posting): 02–05 excerpts written from the dev.to bodies with every claim re-run here, then all 13 scheduled (ids 28017–28029), each 404 logged out. Its "published elsewhere" `source` field is **not** a canonical (page `rel=canonical` stays on coderlegion.com), so excerpts stay the rule. Medium: 02–14 scheduled (ids in the queue README), canonical set in Advanced settings on each (Medium emits it as the real `rel=canonical`, proven on #01), 5 existing topics, 1 code block each. #01's broken copy was replaced and republished, and its canonical was **missing** (Medium-self) and is now set. **Found: 20 of the 22 older Medium stories have no canonical** (open, in NOV-4 guide §G). Paste gotchas are recorded in the queue README. Verified end state: dev.to dashboard 13 Scheduled; Medium Scheduled tab 13 at 13:00Z (11-03 14:00Z); CoderLegion 13/13. The 11/04 post-count gate is now met automatically.
- **2026-09-20 — the cadence was the risk, so it got taken off Travis's calendar.** The 11/04 read turns on post count, not conversion: <10 of 14 shipped = the test never ran = freeze. 13 posts remained against exactly 13 slots (9/22 → 11/3), with zero slack, under WoW beta (→10/21), launch (11/04) and Super Dad (→11/12). Verified against the live account first: **#01 is published** (id `4669851`, 2026-09-16, canonical `/guides/open-ports-linux` correct) — it had simply never been ticked, so the file could not answer "how many shipped". dev.to's **API cannot schedule** (`published_at` is not a create field; confirmed against the Forem API docs) but **its editor can** (hexagon by Publish), so `scripts/devto-drafts.mjs` pushed 02–14 as **unpublished drafts** with title, 4 tags, `canonical_url`, description and cover set from each file's front matter — body-only in `body_markdown`, so nothing leaks. Verified after: 13 drafts, 0 published, canonicals all correct, spot-checked body has no front-matter leak and balanced code fences. Script is idempotent (skips any canonical already on the account) and throttled 12s. **Travis's per-post job drops from ~15 min to ~1 min, once.** Queue README carries the draft-id table and #01's live URL; the missed 9/16 CoderLegion slot is flagged onto Mon 9/21. Human steps, plain text: `docs/MANUAL-ACTIONS-2026-09-20.txt` (scheduling, CoderLegion ×2, the Medium-only Tue/Thu job, Gumroad Discover with the real 30%-vs-10% math, the two awesome-shell PRs, the three numbers to read). Recorded there too: **757 Custom Graphics is the nearer income line and is blocked on this side, not the client's** — ⚠ **corrected later the same day:** the site is **not** undeployed. It has been live at <https://757-custom-graphics.vercel.app> since 2026-09-06; the absent `.vercel/` directory meant only that the folder was never linked, because deploys come from that repo's GitHub integration on `main`. Re-verified 2026-09-20: every page 200 + `noindex, nofollow`, `robots.txt` `Disallow: /`, all internal links resolve, no banned or placeholder strings, and **publicly reachable with no login wall** (cookie-less check — a Vercel auth wall would have shown the client a login screen instead of her website). `{{LIVE_URL}}` in `docs/pitch/PITCH-EMAIL.md` is now filled in. **The only outstanding step is Travis sending the 119-word email, unsent since 9/11.** No site code changed.
- **2026-09-16 (third pass) — queue order, Bing metadata, purchase tracking.** Live check before ordering: dev.to 37 published (newest 7/30), 0 drafts; CoderLegion newest 7/08, 0 drafts. Queue re-ordered by Bing cluster demand and renumbered 01–14 in `docs/cross-posts/queue/` (README = order of record; #01 open ports posted by Travis 9/16, then Tue/Thu to 11/3); 03 → `already-live/`, 12–17 → `reserve/`. The jq CoderLegion excerpt's "misquoted select() fails silently" was false on jq 1.8.1 (compile error, exit 3) and was rewritten to the case that really is silent. M1 shipped: `fitTitle()` + shorter `<title>`s (12 snippet metaTitles, 2 ShellCheck, 5 guides, 4 static pages) and 68 descriptions rewritten → build shows 82 pages, 0 titles > 65, 0 descriptions > 160 (commit `4ce6715`); unbacked "within 60 seconds", "under 100 MB", "any SC code", "tested scripts" dropped. M2 shipped: Gumroad link keeps the referrer (`3f79ff1`); GA4 purchase events are consent-denied pings for most visitors, so **Gumroad views-by-referrer is the purchase-click number of record for 11/04**. HN comments 9/16 logged; Travis starting Reddit comments (rules in POST-QUEUE §F).
- **2026-09-16 (second pass) — fixes shipped.** Travis approved C1 (page matches the zip's single-buyer license), H1–H4 and internal links. `/starter-kit`: no MIT, no "no suppressions", Kali-only test claim, 30-function (hand count: 30 definitions, 27 public), second buy button under the price, both to Gumroad checkout with `placement`. Unbacked Ubuntu/Debian/Fedora/Ventura/RHEL claims removed from homepage, /about, /snippets, 4 snippet descriptions, llms.txt, snippet template. `bashlib-starter`: no signup form, FAQ links `/starter-kit`. Seven orphan snippets linked from their neighbours; built-HTML crawl of 82 pages = 0 broken links, 0 broken fragments, 0 orphans (14 pages have exactly one content inbound). Patched toolkit zip uploaded to Gumroad, v1.0 file removed, saved. Live reads: Bing 3 M = 891 imp / 12 clicks / 472 keywords (9/10: 816/13/448); Copilot citations 142 (9/10: 124); Buttondown = 1 (the test address) → **0 real**; Gumroad 0 sales. kaisrun.xyz is in Bing WMT.
...
- **2026-09-11 (resume, same day) — handoff §2 shipped.** A: `/shellcheck/sc2086` and draft 21 no longer call `find -name $pattern` SC2061 (0.11.0 reports SC2086; with two matches `find` dies with `paths must precede expression`). B: three new snippets + scripts now claim only the Kali run. C: 12 tools, 42 scripts, the §1.4 link. D: Show HN retitled to "31 common SC codes", invented opener and roadmap cut. E (week 1) + F: drafts 21/23 and new 24 (sed) / 25 (trap) opened on real runs from this box, no REVIEW lines. The kit-2 original's "skip the `$?` capture → exit 0" claim is false (only a late capture does that). G: queue re-dated to Fri 9/11 → Wed 9/30, 22 leads; awesome-bash PR unblocked from HN. Bing: sitemap *Success* 9/11 (72 discovered), crawl control maxed, homepage *Indexed*, Request indexing clicked. Flagged, not fixed: homepage title 74 / description 189 chars (Bing errors). Remaining E: backlog drafts 01, 04, 05, 06, 10 before 9/19.
