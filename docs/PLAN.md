...
- **2026-09-11 — identifier leak scrubbed (`99696d2`).** `/snippets/find-ip-address-linux` (shipped `92706cc`) pasted the laptop's real Wi-Fi MAC, the ISP's DNS resolvers and IPv6 subnet bits; `OWNERSHIP.md` + `MANUAL-ACTIONS-2026-09-01.md` held the home public IPv4 + IPv6 /64 (GA4 filter rule) since 9/01. All replaced with documentation values; all 79 live URLs, both working trees, dev.to (37) and Medium swept clean. New `scripts/check-leaks.sh` pre-commit hook (installed in both repos) reads this box's MACs / global IPv6 / resolvers / public IPs live and blocks any staged file containing one. **Open:** the old values remain in public git history of `bashsnippets-next` (20 commits, reachable by SHA) — Travis decides: make the repo private, or rewrite history + force-push.
