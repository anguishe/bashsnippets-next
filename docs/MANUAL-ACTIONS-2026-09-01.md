...
|---|---|---|
| IPv4 | `<HOME_IPV4>` (`curl -s -4 ifconfig.me`) | `<HOME_IPV4>/32` |
| IPv6 | `<HOME_IPV6>` (`curl -s -6 ifconfig.me`) | `<HOME_IPV6_PREFIX>::/64` |
...
(`<HOME_IPV6_PREFIX>::`) is the stable half, and a `/64` is one subnet — it will not catch anyone
else.
...
with `<HOME_IPV6_PREFIX>`. If it stops doing that, the rule has quietly stopped matching.
...
- Rule `Travis Home` exists, `traffic_type` = `internal`, with **both** conditions saved on
  *IP address is in range (CIDR notation)* — `<HOME_IPV6_PREFIX>::/64` and `<HOME_IPV4>/32`.
